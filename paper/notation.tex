...
\ifdefined\R
  \renewcommand{\R}{\mathds{R}}                                             % R, reals
\else
...
\newcommand{\allResampledData}[1][\D]{\mathds{S}_{#1}} % all possible resampled datasets for a specifi inference algorithm on a dataset
\newcommand{\ResampledIndices}{\mathcal{J}} % sequence of pairs of train-test indices (concrete indices for a dataset)
\newcommand{\allResampledIndices}[1][n_\D]{\mathds{J}_{#1}} % all possible resampled indices for a specific mehod on a dataset of given size.
