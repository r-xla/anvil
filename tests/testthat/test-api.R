...
describe("nv_is_finite", {
  it("detects finite values", {
    expect_jit_equal(
      nv_is_finite(nv_tensor(c(1, NaN, Inf, -Inf, 0))),
      nv_tensor(c(TRUE, FALSE, FALSE, FALSE, TRUE))
    )
  })
  it("works via is.finite() generic", {
    expect_jit_equal(
      is.finite(nv_tensor(c(1, NaN, Inf, -Inf, 0))),
      nv_tensor(c(TRUE, FALSE, FALSE, FALSE, TRUE))
    )
  })
})
...
describe("nv_crossprod", {
  it("computes t(x) %*% y", {
    expect_jit_equal(
      ...
      nv_tensor(as.numeric(crossprod(matrix(1:6, 3, 2), matrix(7:12, 3, 2))), shape = c(2, 2), dtype = "f32"),
      tolerance = 1e-5
    )
  })
  it("computes t(x) %*% x when y is NULL", {
    expect_jit_equal(
      ...
      nv_tensor(as.numeric(crossprod(matrix(1:6, 3, 2))), shape = c(2, 2), dtype = "f32"),
      tolerance = 1e-5
    )
  })
...
describe("nv_tcrossprod", {
  it("computes x %*% t(y)", {
    expect_jit_equal(
      ...
      nv_tensor(as.numeric(tcrossprod(matrix(1:6, 2, 3), matrix(7:12, 2, 3))), shape = c(2, 2), dtype = "f32"),
      tolerance = 1e-5
    )
  })
  it("computes x %*% t(x) when y is NULL", {
    expect_jit_equal(
      ...
      nv_tensor(as.numeric(tcrossprod(matrix(1:6, 2, 3))), shape = c(2, 2), dtype = "f32"),
      tolerance = 1e-5
    )
  })
  it("works via S3 generic", {
