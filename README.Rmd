...
{anvl} can be installed from GitHub or [r-universe](https://r-xla.r-universe.dev/builds).
...
Below, we create an R function and `jit()` it.
Then, we call the resulting function on `AnvilArray`s, which will compile and subsequently execute it.
...
[^1]: A real anvil is a tool blacksmiths use to reshape metal; this package reshapes R code in a similar spirit -- tracing, transforming, and recompiling it into efficient forms.
    We spell the package name `anvl` (without the `i`) because `AnVIL` is already taken by a [Bioconductor package](https://bioconductor.org/packages/AnVIL/).
