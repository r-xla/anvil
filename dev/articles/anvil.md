...
`AnvilArray`. It is essentially like an R array, with some differences:
...
We can create an `AnvilArray` from R objects using `nv_array`. Below, we
...
function allows to convert `AnvilArray`s back to R objects. Note that
for 0-dimensional arrays, the result is an R vector of length 1, as R
...
The result of the operation is again an `AnvilArray`.
...
operations applied to `AnvilArray`s, it does not contain the
...
Besides `AnvilArray`s, jit-compiled functions can also take in regular R
...
Static arguments work differently than `AnvilArrays` as the function
...
`AnvilArray`s, although we currently only support (named) lists.
...
`AnvilArray` objects are immutable, i.e., once created, their value
...
1.  Updating an `AnvilArray` that “lives within” a jit-compiled
    function.
2.  Updating an `AnvilArray` “living in R” through a jit-compiled
    function.
