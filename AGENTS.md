...
- **`reverse`** -- Autodiff rules in `R/rules-reverse.R`, built with `reverse_rule()`. The common form is `reverse_rule(function(inputs, outputs, grads, params, required) ...)`: `params` is the call's static parameters (named list); `grads` contains the upstream gradients (one per output); `required` is a logical vector. Return a list of gradients w.r.t. each input, using `NULL` (via `if (required[[i]])`) for inputs that don't need gradients. To customize the forward pass (e.g. save extra intermediates for the backward, like sort indices), use `reverse_rule(forward = function(inputs, params) ...)`: the forward emits primitives via the normal `prim_*` callables, captures intermediates lexically, and returns `list(outputs, backward)` where `outputs` is one box per original call output and `backward` has the same signature as the backward-only form. See `transform_gradient()` for the full contract.
