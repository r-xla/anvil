...
In this vignette, you will learn how to subset arrays in {anvil} and how to update subsets.
Because array shapes in {anvil} programs are static, only certain subsetting operations are supported and they come with some surprises.
...
```{r}
x <- nv_array(1:5)
x[array(c(1, 1, 1))] <- nv_array(c(10L, 20L, 30L))
x
```
