...
1. It supports more datatypes, such as different precisions, as well as unsigned integers.
2. The tensor can live on different *platforms*, such as CPU or GPU.
3. 0-dimensional tensors can be used to represent scalars.
   
We can create such an object from R datatypes using the `nv_tensor` and friends functions.
...
Below, we ommit specifying the platform and datatype, as it will default to `"cpu"` and `"f32"`.
...
alpha <- nv_scalar(rnorm(3), dtype = "f32")
...
We can notice that we only see th `"compiling ..."` message the first and third time.
...
Because the compilation step itself can take some time, {anvil} therefore gives the best results when the same function is called many times with the same input shapes, datatypes, and platforms, or the computation itself is sufficiently large to amortize the compilation overhead.
...
While the latter might not be a big problem for small models, it can lead to significant overhead when training larger models.
Next, we will briefly address the concept of immutability in anvil and how to work around it.

## Immutability

Conceptually, whenever we are defining programs in anvil, we are strictly follwing **value semantics**.
This means, in-place modifications like updating an array element are conceptually not allowed.
...
For the first category, there is function ABC demonstrated below.
...
This means, we are telling the XLA runtime that after we pass those tensors marked as donable into the function, we will no longer use them in R.
...
However, depending on the duration of each iteration, there might be a signficant overhead from switching between R and the compiled code.
