...
If the operation should support automatic differentiation, attach a reverse rule built with `reverse_rule()`.
...
```{r}
prim_repeat_along[["reverse"]] <- reverse_rule(function(inputs, outputs, grads, params, required) {
  if (!required[[1L]]) {
    return(list(NULL))
  }
...
If the gradient computation can be made significantly more efficient by emitting a *different* forward pass that exposes intermediate values for reuse, use the alternative-forward form: `reverse_rule(forward = ...)`.
...
```r
prim_<name>[["reverse"]] <- reverse_rule(forward = function(inputs, params) {
  x <- inputs[[1L]]
...
The `backward` closure shares the same signature as the backward-only form, so the args (`inputs`, `outputs`, `params`) are still passed by the framework.
