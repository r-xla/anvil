...
1. `R` $\rightarrow$ `AnvilGraph`:
   ...
   It operates only on `AnvilArray` objects and applies `anvl::Primitive` operations to them.
...
The {anvl} package does not in any way dictate how such a `AnvilGraph` to `AnvilGraph` transformation can be implemented.
...
For this, `anvl::Primitive` objects have a `$rules` field that can be populated.
...
Here, we will dig deeper into the dichotomy of {anvl} functions such as `nvl_add`.
