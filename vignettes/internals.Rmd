...
1. `R` $\rightarrow$ `AnvilGraph`:
   ...
   It operates only on `AnvilArray` objects and applies `anvil::Primitive` operations to them.
...
To do this, we use `anvil::trace_fn()`, which takes in an `R` function and a list of `AbstractArray` inputs that specify the types of the inputs.
...
To run the function, we need to extract the underlying buffers from the arrays before passing them to the executable, which will output a `PJRTBuffer` that we can easily convert to an `AnvilArray`.
...
The `jit()` function allows to convert a regular `R` function into a Just-In-Time compiled function that can be executed on `AnvilArray`s.
...
For this, the types of all `AnvilArray`s need to match exactly (data type and shape) and all static arguments need to be identical.
For example, if we run the function with `AnvilArray`s of the same type, but different values, the function won't be recompiled, which we can see by checking the size of the cache, which is already 1, because we have called it on `x` and `y` above.
...
Subsequently, the reverse pass will be added to the descriptor, after which it will be converted into a `AnvilGraph`.
...
We can look at this graph below, where `trace_fn` internally converts the `AnvilArray`s `x` and `y` into their abstract representation.
...
2. Currently, constants with the same value (that refer to different `AnvilArray`s) are not deduplicated, which we might change in the future.
...
It is appealing to support the conversion of R literals (`1L`, `1.0` `TRUE`, etc.) to `AnvilArray`s when calling into `jit`-ted functions, i.e. allow the following:

```{r, eval = FALSE}
jit(nv_add, static = character())(1, 2)
```

In `jit`(), we could in principle do this, because we know which arguments are static and which are expected to be `AnvilArray`s.
...
Note that for primitive calls like `nv_array(1)^2` we do auto-convert literals, because we know which arguments are expected to be `AnvilArray`s and otherwise code just becomes much harder to read.

## Dichotomy of anvil functions

Here, we will dig deeper into the dichotomy of {anvil} functions such as `nvl_add`.
In the *Getting Started* vignette, we have learned that these functions can either be called directly on `AnvilArray`s to transform data, or used within `jit()` blocks to build up programs.
Here, we will explain what this actually does and why this is possible.

The core problem this dichotomy solves is that it is a mental burden to always keep two versions os an {anvil} function: 
...
Internally, we have implemented this by wrapping every `nvl_*` primitive function in `jit()` and making a `jit()`ted function behave differently depending on whether we are in another `jit()` call or not.
...
What makes this a little difficult is our multi-backend approach.
Ideally we would just have to `jit()` all the primitives and any `nv_` API function just calls into primitives.
However, this fails in the case, where we create constants in the `nv_` function because these contants assume the `default_backend()` unless specified otherwise.
...
```{r}
add_three <- function(x, dim) {
  three <- nv_scalar(3, dtype = dtype(x))
  nv_add(x, three)
}
```

This function can now unfortunately not be called with both backends and we therefore have to wrap it in `jit()` which means `nv_scalar()` an infer the correct backend from the jit context.


```{r}
add_one <- jit(function(x) {
  n
  n
  one <- nv_const_like(x, )
  nv_array(1, shape = shape(x))
  nvl_add(x, one)
}, backend = "auto")

add_one(nv_array(1, backend = "quickr))
```
...
jit(gradient(f, wrt = c("x", "y")), static = "which")(nv_scalar(1), nv_scalar(2), TRUE)
```
