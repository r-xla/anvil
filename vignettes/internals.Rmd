...
Note that the `nv_mul` is itself not primitive, but performs some type promotion and broadcasting if needed, before calling into the primitive `prim$mul`.
...
Here, we will dig deeper into the dichotomy of {anvil} functions such as `prim$add`.
...
```{r}
library(anvil)
prim$add(nv_scalar(1), nv_scalar(2))
