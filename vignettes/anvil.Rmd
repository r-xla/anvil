...
Below, we omit specifying the platform and data type, as it will default to `"cpu"`.
The default data type depends on the input data type.
It is `"f32"` for R doubles and `"i32"` for integers.
...
One current restriction of {anvil} is that the function has to be re-compiled for every unique combination of input types, each consisting of a specific shape and data type.
...
How important this overhead is depends on how expensive each call in the loop is -- for expensive functions the overhead becomes negligible.
...
1. A `cond` function, which takes as input the current state and returns a boolean tensor indicating
