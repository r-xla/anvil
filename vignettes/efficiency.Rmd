...
There might be some exceptions to this, as in some cases, compilation of a bigger function might be more costly than compilation of many separate functions as there is less room for joint optimizations.
...
- **Copying data between R and the GPU is slow.** Each transfer has a fixed overhead plus a per-byte cost. Once you've moved an array onto the GPU, keep it there; avoid `as_array()` (which copies the value back into R) inside loops you want to run fast (see *Writing efficient loops* below).
...
Speedups between `f32` and `f64` also depend on the specific hardware.
...
One common strategy is to split up the varying lengths into buckets of size `2^n` to balance re-compilation with wasted computation.
The example above only works because the padded zeros are the *neutral value* of the performed computation (addition).
Other common neutral values are:
...
For anything that *does* depend on the padded positions, such as the mean, a little more care is needed: the padded values have to be masked out explicitly.
This can be done by passing the length of the original (non-padded) input along as an argument to the function:

```{r}
masked_mean <- jit(function(x, n) {
  idx  <- nv_iota(dim = 1L, dtype = "i32", shape = shape(x))
  mask <- nv_convert(idx <= n, "f32")
  nv_reduce_sum(x * mask, dims = 1L) / n
})

x_padded <- nv_array(c(1, 2, 3, 4, 0, 0, 0, 0), dtype = "f32")
masked_mean(x_padded, nv_scalar(4L, "i32"))
masked_mean(x_padded, nv_scalar(2L, "i32"))
```

The mask zeroes out padded positions before summing, and the divisor is the true length `n` rather than the padded length.
Because `n` is a regular argument (not `static`), varying it across calls does not trigger any recompilation.
...
The natural question is how we avoid reading wrong results when functions always immediately return without the computation being finished.
This works out because {anvl} will await the computation whenever it actually needs to read the data.
This, e.g., happens when we print it or convert back to R via `as_array()`.
Some operations such as accessing the `dtype()`, `shape()` or `device()` also work without awaiting the result.
...
And this is what will in fact happen when you write code like the one below:
Because the GPU call returns immediately, the R loop will continue preparing the next batch while the GPU computation from the previous iteration is not even done.
In the next iteration we will call into the `step()` function again and the computation will be scheduled to run on the GPU.
Note that the GPU computation from the previous `step()` call might not even be done at this point and this is exactly the situation we want to be in, as we don't want the GPU to idle.
...
One way to make this loop much worse is to always print the result after each `step()`:
...
for (i in seq_len(n_steps)) {
  batch   <- prepare_batch(i)                # CPU-work
  weights <- step(batch$X, weights, batch$y) # GPU-work
  cat("loss:", as_array(loss(batch$X, weights, batch$y)), "\n")
}
```

Because `as_array()` requires the actual data, we now have to wait for the GPU to finish its computation so the result can be sent back to the CPU.
But this means that while the next batch is being prepared, the GPU is idle.
Solutions to this problem are, e.g., logging less often or printing the loss from the previous iteration.
...
If you know a particular input will not be used after the call, you can mark it as `donate` so XLA is free to reuse its memory for the output:
