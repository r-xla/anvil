...
- `inputs`: (`list<GraphVariable>`) The function inputs.
- `outputs`: (`list<GraphVariable>`) The function outputs.
- `calls`: (`list<PrimitiveCall>`) A list of `PrimitiveCall` objects that represent the body of the function.
  A primitive call takes the values from some input `GraphVariable`s (plus some, optional, static parameters) and assigns the results to some output `GraphVariable`s.
...
```{r}
x_gvar <- GraphVariable(ShapedTensor(dt_i32, Shape(c(2, 2))))
y_gvar <- GraphVariable(ShapedTensor(dt_i32, Shape(c(2, 2))))
```
