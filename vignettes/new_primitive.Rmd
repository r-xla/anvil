...
- Pass the primitive **name string** to `graph_desc_add()`; it looks up the primitive in the internal registry.
...
`new_primitive()` returns the callable directly and also registers the primitive into an internal registry used by the graph machinery, so no separate registration step is needed:
