...
Primitives are created with `new_primitive()`: it builds the `AnvilPrimitive` metadata object that holds the rules, wraps the body with `jit()`, attaches the metadata, and registers the result in the internal primitive registry. The returned callable becomes the primitive and is bound to a `prim_<name>` R symbol.
...
prim_repeat_along <- new_primitive(
  ...
  function(operand, times, dim) {
    ...
    graph_desc_add(
      self,                       # lexically bound to the AnvilPrimitive
...
- Pass the lexically-bound `self` (the [`AnvilPrimitive`]) as the first argument to `graph_desc_add()`. `new_primitive()` installs `self` into the enclosing environment of the body, so this is always available inside a primitive.
