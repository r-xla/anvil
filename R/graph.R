...
# Bulk variant of register_input: register each gval as a graph input of `desc`.
...
# Bulk variant of get_box_or_register_const: ensure each gval has a box in
# `desc`, registering it as a constant if not already present. Used for graph
# constants and -- when inlining a graph into a parent descriptor -- also for
# the inlined graph's "inputs" (which are values supplied by the parent and
# therefore behave like constants in the parent).
