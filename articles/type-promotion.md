...
The type of `z` is an ambiguous `i32`, because `x` is promoted to an
`i32`, the default type of the `1L` literal. If `z` was not ambiguous,
the output would be an `i32`, because the `y` would be promoted to an
`i32` in the multiplication. Because we propagate the ambiguity, the `z`
is actually down-promoted to an `i16`, because the `z` is ambiguous,
while the `y` is known.
