...
struct GpuLibs {
  ...
  int (*d_geqrf)(void*, int, int, double*, int, double*, double*, int, int*);

  // cuSOLVER orgqr (float)
  int (*s_orgqr_bs)(void*, int, int, int, const float*, int, const float*,
                    int*);
  int (*s_orgqr)(void*, int, int, int, float*, int, const float*, float*, int,
                 int*);
...
static Error qr_cuda_impl(void* stream, AnyBuffer input,
  ...
  int status = Solver<T>::geqrf(g)(
      handle, m, n, reinterpret_cast<T*>(d_a.ptr), m,
      reinterpret_cast<T*>(d_tau.ptr), reinterpret_cast<T*>(d_work.ptr), lwork,
  ...
  int lwork_orgqr = 0;
  Solver<T>::orgqr_bs(g)(handle, m, k, k,
  ...
  status = Solver<T>::orgqr(g)(
      handle, m, k, k, reinterpret_cast<T*>(q_ptr), m,
      reinterpret_cast<const T*>(d_tau.ptr), work_ptr, lwork_orgqr,
      reinterpret_cast<int*>(d_info.ptr));
  if (status != 0) {
...
static Error do_qr_cuda(void* stream, AnyBuffer input,
                         Result<AnyBuffer> q_out, Result<AnyBuffer> r_out) {
