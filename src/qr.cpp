...
// column-major conversion transparently — the handler works directly with
...
Error qr_impl<double>(AnyBuffer input, Result<AnyBuffer> q_out,
  ...
  std::vector<double> a(static_cast<const double *>(input.untyped_data()),
                        static_cast<const double *>(input.untyped_data()) +
                            m * n);
...
Error qr_impl<float>(AnyBuffer input, Result<AnyBuffer> q_out,
  ...
  for (int i = 0; i < m * k; i++)
    q_f32[i] = static_cast<float>(a[i]);
  for (int i = 0; i < k * n; i++)
    r_f32[i] = static_cast<float>(r_f64[i]);
...
Error qr_impl<float>(AnyBuffer input, Result<AnyBuffer> q_out,
  ...
  std::vector<float> a(static_cast<const float *>(input.untyped_data()),
                       static_cast<const float *>(input.untyped_data()) +
                           m * n);
